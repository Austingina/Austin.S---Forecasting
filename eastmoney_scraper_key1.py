...
class EastmoneyReportScraper:
    ...
    def get_data_from_api(self, begin_time, end_time, keyword, page_size, page_num=1):
        ...
        # Convert keyword list to string separated by spaces
    ...
    # Check if title or stock name contains keywords
    def contains_keyword(self, title, stockName, keyword):
        for kw in keyword:
            if kw.lower() in title.lower() or kw.lower() in stockName.lower():
                return True
        return False

    def scrape(self, keyword, start_date, end_date, output_file, desired_count):
        start_date = datetime.strptime(start_date, "%Y-%m-%d")
        end_date = datetime.strptime(end_date, "%Y-%m-%d")
        records = []
        driver = self.configure_browser()
        # Create target folder named after keywords
        keyword_str = self.sanitize_filename("_".join(keyword) if isinstance(keyword, list) else keyword)
        target_folder = os.path.abspath(keyword_str)
        ...
        while current_date <= end_date:
            ...
            while total_fetched_today < desired_count:
                ...
                json_data = self.get_data_from_api(begin_time_str, end_time_str, keyword, per_page, page_num)
                ...
                for item in data_list:
                    ...
                    # Check if title or stock name contains keywords
                    if not self.contains_keyword(title, stockName, keyword):
                        continue
